...
# EZEK A RANGOK LESZNEK KITILTVA A BAN/KICK HASZNÁLATÁBÓL:
...
# MÓDOSÍTOTT ELLENŐRZŐ: KIFEJEZETTEN KIZÁRJA A KÉT RANGOT A BAN/KICK-BŐL
def high_mod_check(i: discord.Interaction):
    user_role_ids = [role.id for role in i.user.roles]
    # Ha a felhasználónak van bármelyik tiltott rangja, akkor FALSE (nem használhatja)
    if TESTER_MOD_ID in user_role_ids or MOD_ID in user_role_ids:
        return False
    # Egyébként csak az Adminisztrátorok használhatják
...
class GiveawayModal(ui.Modal, title='Nyereményjáték Beállítása'):
    ...
    prize = ui.TextInput(label='Mi a nyeremény?', placeholder='Írd ide...', required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="⏳ Hátralévő idő", value=f"<t:{end_ts}:R> múlva ér véget", inline=True)
        ...
        try:
            dm_embed = make_embed("🎫 Nyereményjáték Létrehozva", f"Nyeremény: **{self.prize.value}**\n🆔 Üzenet ID: `{msg.id}`", discord.Color.green())
        ...
        if users:
            ...
            await interaction.channel.send(f"😢 A(z) **{self.prize.value}** sorsolása sikertelen (nem maradt jelentkező).")
...
# TOVÁBBI ALAP PARANCSOK (Mute, Unmute, Törlés, Beállítások)
@bot.tree.command(name="üdvözlő_beállítás")
@app_commands.check(mod_check)
async def welcome_set(i, csatorna: discord.TextChannel):
    save_json(WELCOME_FILE, {"channel_id": csatorna.id}); await i.response.send_message("✅ Üdvözlő kész!", ephemeral=True)

@bot.tree.command(name="kilépő_beállítás")
@app_commands.check(mod_check)
async def leave_set(i, csatorna: discord.TextChannel):
    save_json(LEAVE_FILE, {"channel_id": csatorna.id}); await i.response.send_message("✅ Kilépő kész!", ephemeral=True)

@bot.tree.command(name="autorole_beállítás")
@app_commands.check(mod_check)
async def autorole_set(i, rang: discord.Role):
    save_json(AUTO_ROLE_FILE, {"role_id": rang.id}); await i.response.send_message("✅ Autorole kész!", ephemeral=True)
...
async def unmute(i, tag: discord.Member):
    await tag.timeout(None); await i.response.send_message(embed=make_embed("🔊 Feloldva", f"👤 {tag.mention}\n👮‍♂️ {i.user.mention}", discord.Color.green()))
...
async def warn_del(i, tag: discord.Member, szám: int):
    ...
    if 0 < szám <= len(warns):
        ...
        await i.response.send_message(embed=make_embed("🧹 Törlés", f"👤 {tag.mention}\n👮‍♂️ {i.user.mention}", discord.Color.green()))
